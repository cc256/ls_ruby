class Length
  ...
  def <=>(other)                                                  # add method.
    case unit
    when :km  then value <=> other.as_kilometers.value            # add <=> operator     
    when :mi  then value <=> other.as_miles.value                 # https://stackoverflow.com/questions/827649/what-is-the-ruby-spaceship-operator
    when :nmi then value <=> other.as_nautical_miles.value        # one way. https://docs.ruby-lang.org/en/2.5.0/Array.html
    end                                                           # https://docs.ruby-lang.org/en/2.5.0/Comparable.html      
  end
